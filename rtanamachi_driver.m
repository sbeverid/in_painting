function [output] = rtanamachi_driver(z)
%%%%%%%%%%%%
...
%inp = 'S20160307T020003.nc.txt';       %clear sky (uses method 1: all_negatives)
...
%        method = 2;
%end
[noise_removed, mask, new_z] = rtanamachi_scaling(z); % RLT20200106


%4: method calls prof chans inpainting method
[inpainted] = gdennany_inpaint(noise_removed, mask);

% Replaces all data points not inpainted with original data values in order 
% to remove "graininess" from inpainted data
[s1,s2] = size(mask);                     % Get size of mask dimensions
for n = 1:s1                              % Loop for each row
    for m = 1:s2                          % Loop for each value in row
        if mask(n,m) == 1                 % If mask value at location n,m is 1 continue
            inpainted(n,m) = new_z(n,m);  % Replace inpainted value with original data (new_z)
        end
    end
end
...
%6: Uses inward interpolation on 1/2 sized image to fill in any remaining holes
...
%    for row = 1 : numRows
%        for column = 1 : numColumns
%            inpainted(row,column) = ((inpainted(row,column) - .5) * 100);
%        end
%    end
%end
maximum = 47.6;       
output = 98.1 * output - 50.5; % Inverts the scaling function

%imshowpair(z, output, 'montage');


%ubplot(1,1,1), imshow(z);
